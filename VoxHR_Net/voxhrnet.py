...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsample all lower resolution features to the highest resolution
        x0_shape = x[0].shape[-3:]
        for i in range(1, len(x)):
            x[i] = F.interpolate(x[i], size=x0_shape, mode='trilinear', align_corners=False)
        
        x = torch.cat(x, 1)
        x = self.last_layer(x)
        # print(f"[HRNet] After last_layer, shape: {x.shape}")
        
        # 性能优化：如果尺寸相同则跳过插值
        if x.shape[-3:] != original_shape[-3:]:
            x = F.interpolate(x, size=original_shape[-3:], mode='trilinear', align_corners=False)
        # print(f"[HRNet] Final output shape: {x.shape}")
